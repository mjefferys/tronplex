...
function sendDownloadProgressToUpdateWindow(percent) {
  log.info(percent);
...
autoUpdater.on("error", err => {
  ...
  sendStatusToUpdateWindow(
    "Error in auto-updater. Please restart Tronplex. If this continues submit a bug."
  );
});
...
autoUpdater.on("update-downloaded", info => {
  ...
  function doUpdate(result) {
    if (result) {
      ...
      sendStatusToUpdateWindow(
        "Chose not to update, will automatically update on next restart"
      );
      updateWindow.hide();
    }
  }
